--[[
@module  modbus_win
@summary Modbus RTU 主站 - 通讯管理页面模块
@version 1.0
@date    2026.05.13
@usage
本模块为通讯管理页面，显示 Modbus RTU 主站配置入口。
订阅"OPEN_MODBUS_WIN"事件打开窗口。
]]

local modbus_rtu_master = require("modbus_rtu_master")
...
-- ========================================
-- 校验位选项（与 modbus_rtu_master.lua 保持一致）
-- ========================================
...
local function rtu_apply_config(config)
    ...
    if rtu_config_inputs["slave_id"] and rtu_config_inputs["slave_id"].ui then
        rtu_config_inputs["slave_id"].ui:set_text(tostring(config.slave_id or 1))
    end
    if rtu_config_inputs["poll_interval"] and rtu_config_inputs["poll_interval"].ui then
        rtu_config_inputs["poll_interval"].ui:set_text(tostring(config.poll_interval or 1000))
    end
    if rtu_config_inputs["timeout"] and rtu_config_inputs["timeout"].ui then
        rtu_config_inputs["timeout"].ui:set_text(tostring(config.timeout or 3000))
    end
...
local function rtu_validate_config()
    ...
    local slave_id_input = rtu_config_inputs["slave_id"]
    ...
    if rtu_config_inputs["poll_interval"] and rtu_config_inputs["poll_interval"].ui then
        poll_interval = tonumber(rtu_config_inputs["poll_interval"].ui:get_text()) or 1000
    end
    if poll_interval < 1000 then
        airui.msgbox({
            ...
            on_action = function(self, label)
                if label == "好的" then self:hide() end
            end
        }):show()
        return
    end
    ...
    local timeout = 3000
    if rtu_config_inputs["timeout"] and rtu_config_inputs["timeout"].ui then
        timeout = tonumber(rtu_config_inputs["timeout"].ui:get_text()) or 3000
    end
    ...
    modbus_rtu_master.save_config(config)
...
local function add_register_row(addr, name, value, rw)
    ...
    modbus_rtu_master.add_register(addr, name, value, rw_val)
    ...
    if register_row_map[addr] then
        ...
        rtu_reg_table:set_cell_text(row, 3, rw_text)
    ...
    airui.button({
        ...
        on_click = function()
            ...
            if addr and register_row_map[addr] then
                ...
                modbus_rtu_master.remove_register(addr)
            end
            close_register_dialog()
        end
    })
...
local function create_register_table(parent, y)
    ...
    rtu_reg_table:set_cell_text(0, 3, "读/写")
end
...
sys.subscribe("modbus_log", function(data)
    if data.port_type == "rtu_master" then
...
local function create_config_card(parent, y)
    ...
    rtu_toggle_btn = airui.button({
        ...
        on_click = function()
            ...
            if rtu_is_open then
                log.info("modbus_rtu", "启动RTU主站")
                local ok, err = modbus_rtu_master.start("rtu_master")
                if not ok then
                ...
                log.info("modbus_rtu", "停止RTU主站")
                local ok, err = modbus_rtu_master.stop("rtu_master")
                if not ok then
    ...
    rtu_config_inputs["slave_id"] = {ui = airui.textarea({
        parent = card1_container,
        x = 170,
        y = 185,
        w = 120,
    ...
    rtu_config_inputs["poll_interval"] = {ui = airui.textarea({
        parent = card1_container,
        x = 170,
        y = 220,
        w = 120,
    ...
    rtu_config_inputs["timeout"] = {ui = airui.textarea({
        parent = card1_container,
        x = 170,
        y = 255,
        w = 120,
...
local function create_ui()
    ...
    -- 参数配置卡片：y=10，卡片高度=405，结束于y=440
    create_config_card(content_container, 10)
    -- 寄存器映射表：y=450，卡片高度=170，结束于y=620
    create_register_table(content_container, 450)
    -- 通讯日志：y=650，容器y=675
    create_log_card(content_container, 650, 675)
end
...
local function on_destroy()
    ...
    if rtu_is_open then
        log.info("modbus_rtu", "主站正在运行，先停止")
        modbus_rtu_master.stop("rtu_master")
        rtu_is_open = false
    end
