...
local function add_saved_network(ssid, password, adv_config)
    ...
    local entry = {ssid = ssid, password = password}
    if adv_config then
    ...
    if ok then
        ...
        sys.subscribe("WIFI_STORAGE_SAVE_REQ", function(data)
            ...
            if data.ssid and data.ssid ~= "" and data.password then
                add_saved_network(data.ssid, data.password, data.advanced_config)
            end
        ...
        sys.subscribe("WIFI_STORAGE_ADD_TO_SAVED_LIST_REQ", function(data)
            add_saved_network(data.ssid, data.password)
        end)
