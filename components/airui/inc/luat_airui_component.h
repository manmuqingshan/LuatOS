...
int airui_container_set_color(lv_obj_t *container, uint32_t color, int opacity); //设置背景颜色和可选透明度（0-255）
...
const char *airui_table_get_cell_text(lv_obj_t *table, uint16_t row, uint16_t col); //获取单元格文本
...
const char *airui_win_get_title(lv_obj_t *win); //获取窗口标题
