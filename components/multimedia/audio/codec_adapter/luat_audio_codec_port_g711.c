...
static void g711_codec_destroy(luat_audio_data_codec_t* coder) {
...
static int g711_codec_get_info(luat_audio_data_codec_t* coder, FILE* fd) {
    (void)fd;

    // G711固定参数：8kHz采样率, 单声道, 8位深度
    coder->common_param.sample_rate = 8000;
    coder->common_param.num_channels = 1;
    coder->common_param.bits_per_sample = 8;
    coder->common_param.audio_format = LUAT_MULTIMEDIA_DATA_TYPE_PCM;

    return 1;
}

static int g711_codec_decode_file_data(luat_audio_data_codec_t* coder, luat_zbuff_t* out_buff, uint32_t mini_output) {      
