...
void luat_opus_decoder_destroy(luat_audio_data_codec_t *coder){
    if (coder->ctx) {
...
int luat_opus_decoder_get_data(luat_audio_data_codec_t *coder, const uint8_t* input, uint32_t len,
...
int luat_opus_encoder_create(luat_audio_data_codec_t *coder){
    int error;
...
void luat_opus_encoder_destroy(luat_audio_data_codec_t *coder){
    if (coder->ctx) {
...
int luat_opus_encoder_get_data(luat_audio_data_codec_t *coder, const int16_t* pcm, uint32_t len,
...
static void* opus_codec_create(luat_audio_data_codec_t* coder) {
    if (coder->is_decoder) {
...
static void opus_codec_destroy(luat_audio_data_codec_t* coder) {
    if (!coder) return;
