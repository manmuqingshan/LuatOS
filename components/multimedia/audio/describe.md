...
| `LUAT_AUDIO_DRIVER_TYPE_*` | `I2S=0, DAC, ADC, DAC_ADC, DAC_I2S, ADC_I2S, USB` | 硬件接口类型 |
...
| `LUAT_AUDIO_DATA_CODEC_TYPE_*` | `WAV=0, AMR_NB, AMR_WB, TTS, MP3, OPUS, G711, MAX` | 编解码器标识 |
...
| 流播放 | `play_buff / one_block_len / block_nums` | 流式数据缓冲区参数 |
...
```
luat_audio_request_play_stream() [通过 prepare + start 手动构建]:
  ├─ prepare → is_stream = 1
  ├─ 用户指定 codec（或自动搜索）
  ├─ modify_audio_common_param
...
| `luat_audio_request.h` | 9 | `play_files`, `play_tts`, `record`(⚠未实现), `speech`(⚠未实现), `prepare`, `cancel`, `init`, `deinit`, `start` |

总 API 数量：约 **40** 个公开函数（其中 2 个声明但未实现）。
...
-- 4. 暂停/恢复控制
sys.wait(5000)
audio_v2.pause(req_id, true)   -- 暂停
sys.wait(2000)
audio_v2.pause(req_id, false)  -- 恢复
