...
static int _audio_tts_output_callback(void *data, uint32_t param, void *user_data)
{
	...
	if (data) {
		while(!request_block->is_user_stop && luat_fifo_check_free_space(request_block->data_channel->play_fifo) >= request_block->codec.opts->decode_max_output_len * ((request_block->data_channel->driver_ctrl->data_align == 3)?4:request_block->data_channel->driver_ctrl->data_align))
		{
...
static int _audio_start_request(luat_audio_request_block_t *request_block)
{
	...
	if (request_block->data_channel->driver_ctrl->opts->rx_interrupt_switch) {
		request_block->data_channel->driver_ctrl->opts->rx_interrupt_switch(request_block->data_channel->driver_ctrl, 0);
	}
	...
	// 最后根据请求块的模式做不同的解码操作
	if (request_block->is_stream) {		//流媒体模式直接启动
