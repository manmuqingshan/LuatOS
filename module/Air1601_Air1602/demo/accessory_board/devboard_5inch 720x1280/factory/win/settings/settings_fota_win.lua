...
local function update_screen_size()
    ...
    input_w  = math.floor(card_w * 0.16)
...
local num_keyboard = nil

local function create_num_keyboard()
    if num_keyboard then return num_keyboard end
    num_keyboard = airui.keyboard({
        x = 0, y = screen_h,
...
local function build_ui()
    ...
    airui.label({
        ...
        x = math.floor(card_w * 0.05), y = row1_y,
        w = label_w, h = input_h,
        text = "开机自动检测",
    ...
    airui.label({
        parent = settings_card,
        x = math.floor(card_w * 0.05), y = row2_y,
        w = label_w, h = input_h,
        text = "检测间隔(小时)",
        font_size = font_size2,
        color = COLOR_TEXT,
    ...
    interval_input = airui.textarea({
        parent = settings_card,
        x = label_w + math.floor(card_w * 0.05), y = row2_y,
        w = input_w, h = input_h,
        text = "24",
        font_size = font_size2,
        input_type = "number",
    ...
    airui.label({
        ...
        y = row2_y,
        w = math.floor(card_w * 0.15), h = input_h,
        text = "小时",
        font_size = font_size2,
    ...
    status_label = airui.label({
        parent = content_area,
        x = margin, y = y,
        w = card_w, h = math.floor(screen_h * 0.05),
        text = "就绪",
        font_size = font_size2,
        color = COLOR_TEXT_SECONDARY,
