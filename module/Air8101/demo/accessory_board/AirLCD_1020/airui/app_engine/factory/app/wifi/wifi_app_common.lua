...
function M.handle_ip_ready(ip, ad, ws, rf)
    if ad == socket.LWIP_STA then
        log.info("wifi_app_common", "WiFi IP就绪:", ip)
        rf()
    end
end
...
function M.handle_ip_lose(ad, ws)
    if ad == socket.LWIP_STA then
        log.info("wifi_app_common", "WiFi IP断开")
        ws.ready = false
        ws.ip = "--"
        ws.netmask = "--"
        ws.gateway = "--"
...
function M.handle_scan_done(ws, sr, od)
    ...
    log.info("wifi_app_common", "扫描完成，找到", #ws.scan_results, "个热点")
    if od then od() end
end
...
function M.handle_scan_timeout(sr, ot)
    ...
    log.warn("wifi_app_common", "扫描超时")
    if ot then ot() end
end

function M.auto_scan_and_verify(sc, sto)
    sto = sto or 20000
    log.info("wifi_app_common", "开始自动扫描并查找最佳已保存网络")
    ...
    if bs then
        log.info("wifi_app_common", "找到最佳已保存网络:", bs, "信号:", br)
        return { verified = true, ssid = bs, password = bp, signal = br, config = bc }
    end
    log.info("wifi_app_common", "未在附近找到任何已保存网络")
    return { verified = false }
end

function M.on_storage_set_enabled_rsp(dt, sc)
    log.info("wifi_app_common", "设置开关响应:", dt.success, dt.enabled)
    if sc then
...
function M.on_get_status_req(ws, sc)
    log.info("wifi_app_common", "收到获取状态请求")
    local st = {}
...
function M.on_storage_get_saved_list_rsp(dt)
    log.info("wifi_app_common", "收到已保存网络列表，数量:", #dt.list)
    sys.publish("WIFI_SAVED_LIST_RSP", {list = dt.list})
end

function M.on_storage_load_rsp(dt, sc, af)
    sc = dt.config
    log.info("wifi_app_common", "加载配置完成:", sc.ssid, "enabled:", sc.wifi_enabled)
    sys.taskInit(af)
end

function M.on_storage_init_rsp(dt, ons)
    log.info("wifi_app_common", "storage初始化响应:", dt.success)
    if not dt.success then
        log.error("wifi_app_common", "storage初始化失败")
        return
    end
    sys.publish("WIFI_STORAGE_LOAD_REQ")
    log.info("wifi_app_common", "初始化完成")
    if ons then ons() end
end

function M.init(sc, osf)
    log.info("wifi_app_common", "开始初始化")
